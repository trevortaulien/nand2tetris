...
def removeCommentsAndWhitespace(lines):
...
def handleSymbols(lines):
    ...
    def documentLabelSymbols(lines, symbolTable):
        for location, line in enumerate(lines):
            if(line[0] == ('(')):
                symbol = line.strip('()')
                while(lines[location + 1][0] == '('):           # Covers condition when multiple label symbols are back to back
                    location = location + 1
...
lines = removeCommentsAndWhitespace(lines)
lines = handleSymbols(lines)
